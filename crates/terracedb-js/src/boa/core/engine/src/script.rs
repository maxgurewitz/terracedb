...
fn wrap_script_suspend(continuation: NativeResume) -> NativeResume {
    NativeResume::from_copy_closure_with_captures(resume_script_suspend, continuation)
}

fn resume_script_suspend(
...
impl Script {
    ...
    pub fn evaluate(&self, context: &mut Context) -> JsResult<JsValue> {
        match self.evaluate_interruptible(context)? {
            InterruptibleCallOutcome::Complete(value) => Ok(value),
            InterruptibleCallOutcome::Suspend(_) => Err(crate::JsNativeError::error()
                .with_message("script evaluation suspended during synchronous execution")
                .into()),
        }
    }
    ...
    pub fn evaluate_interruptible(
        ...
        if matches!(record, crate::vm::CompletionRecord::Suspend) {
            ...
            return Ok(InterruptibleCallOutcome::Suspend(wrap_script_suspend(
                continuation,
            )));
        }
        ...
        record.consume().map(InterruptibleCallOutcome::Complete)
    }

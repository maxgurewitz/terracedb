...
impl EnvironmentStack {
    ...
    pub(crate) fn push_lexical(
        ...
        self.push_env(Environment::Declarative(Gc::try_new(
    ...
    pub(crate) fn push_module(&mut self, scope: Scope) -> JsResult<()> {
        let num_bindings = scope.num_bindings_non_local();
        self.push_env(Environment::Declarative(Gc::try_new(
            DeclarativeEnvironment::new(
    ...
    fn push_env(&mut self, env: Environment) -> JsResult<()> {
        self.tip = Some(Gc::try_new(EnvironmentNode {
            env,

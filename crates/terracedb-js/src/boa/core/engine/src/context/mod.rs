...
use std::{cell::{Cell, RefCell}, path::Path, rc::Rc};
...
use crate::{
    ...
    object::{FunctionObjectBuilder, InterruptibleCallOutcome, JsObject, shape::RootShape},
...
impl Context {
    ...
    pub async fn run_jobs_async(&mut self) -> JsResult<()> {
        self.job_executor().run_jobs_async(&RefCell::new(self)).await
    }

    /// Returns whether this context has suspended execution waiting on the embedder.
    #[inline]
    #[must_use]
    pub fn has_suspended_execution(&self) -> bool {
        self.vm.suspended_resume.is_some()
    }

    /// Resumes suspended execution with a successful host operation completion.
    #[inline]
    pub fn resume_interruptible(&mut self) -> JsResult<InterruptibleCallOutcome<JsValue>> {
        self.resume_interruptible_with(crate::vm::CompletionRecord::Normal(JsValue::undefined()))
    }

    /// Resumes suspended execution with a failed host operation completion.
    #[inline]
    pub fn resume_interruptible_with_error(
        &mut self,
        error: crate::JsError,
    ) -> JsResult<InterruptibleCallOutcome<JsValue>> {
        self.resume_interruptible_with(crate::vm::CompletionRecord::Throw(error))
    }

    /// Resumes suspended constructor execution.
    #[inline]
    pub fn resume_interruptible_construct(&mut self) -> JsResult<InterruptibleCallOutcome<JsObject>> {
        match self.resume_interruptible()? {
            InterruptibleCallOutcome::Complete(value) => Ok(InterruptibleCallOutcome::Complete(
                value.as_object()
                    .ok_or_else(|| {
                        JsNativeError::error()
                            .with_message("resumed constructor did not return an object")
                    })?
                    .clone(),
            )),
    ...
    pub fn resume_interruptible_with(
        ...
        interrupt_trace(|| {
            format!(
                "resume_interruptible_with start suspended={} frames={}",
                self.vm.suspended_resume.is_some(),
                self.vm.frames.len(),
            )
        });
        let Some(continuation) = self.vm.suspended_resume.take() else {
            interrupt_trace(|| {
                format!(
                    "resume_interruptible_with missing suspended execution frames={}",
                    self.vm.frames.len(),
                )
            });
            return Err(JsNativeError::error()
                .with_message("no suspended execution to resume")
                .into());
        };
        ...
        match continuation.call(completion, self)? {
            ...
            crate::native_function::NativeFunctionResult::Suspend(next) => {
                interrupt_trace(|| {
                    format!(
                        "resume_interruptible_with result=suspend frames={}",
                        self.vm.frames.len(),
                    )
                });
                self.vm.suspended_resume = Some(next.clone());
                Ok(InterruptibleCallOutcome::Suspend(next))
            }
        }
    }

    /// Installs an interruptible continuation for a later call to `resume_interruptible`.
    #[inline]
    pub fn install_interruptible_resume(&mut self, continuation: crate::native_function::NativeResume) {
        self.vm.suspended_resume = Some(continuation);
    }

    pub(crate) fn continue_interruptible_vm(&mut self) -> JsResult<NativeFunctionResult> {
        let record = self.run();
        if matches!(record, crate::vm::CompletionRecord::Suspend) {

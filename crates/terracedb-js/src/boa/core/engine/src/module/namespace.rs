...
use crate::object::internal_methods::{
    ...
    ordinary_define_own_property, ordinary_delete, ordinary_get, ordinary_get_interruptible,
    ordinary_get_own_property, ordinary_has_property, ordinary_has_property_interruptible,
    ordinary_own_property_keys, ordinary_set_interruptible, ordinary_try_get,
    ordinary_try_get_interruptible,
};
...
impl JsData for ModuleNamespace {
    fn internal_methods(&self) -> &'static InternalObjectMethods {
        static METHODS: InternalObjectMethods = InternalObjectMethods {
            ...
            __has_property_interruptible__: module_namespace_exotic_has_property_interruptible,
            __try_get__: module_namespace_exotic_try_get,
            __try_get_interruptible__: module_namespace_exotic_try_get_interruptible,
            __get__: module_namespace_exotic_get,
            __get_interruptible__: module_namespace_exotic_get_interruptible,
            __set__: module_namespace_exotic_set,
            __set_interruptible__: module_namespace_exotic_set_interruptible,
...
fn module_namespace_exotic_has_property(
    ...
    match module_namespace_exotic_has_property_interruptible(obj, key, context)? {
...
fn module_namespace_exotic_has_property_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    context: &mut InternalMethodPropertyContext<'_>,
    ...
    let key = match key {
        PropertyKey::Symbol(_) => return ordinary_has_property_interruptible(obj, key, context),
        PropertyKey::Index(idx) => js_string!(format!("{}", idx.get())),
        PropertyKey::String(s) => s.clone(),
    };
    ...
    // 4. Return false.
    Ok(ExecutionOutcome::Complete(exports.contains(&key)))
}
...
fn module_namespace_exotic_try_get(
    ...
    match module_namespace_exotic_try_get_interruptible(obj, key, receiver, context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
        ExecutionOutcome::Suspend(_) => Err(JsObject::suspended_internal_method_error(
            "property access",
        )),
...
fn module_namespace_exotic_try_get_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    receiver: JsValue,
    context: &mut InternalMethodPropertyContext<'_>,
    ...
    let key = match key {
        PropertyKey::Symbol(_) => return ordinary_try_get_interruptible(obj, key, receiver, context),
        PropertyKey::Index(idx) => js_string!(format!("{}", idx.get())),
        PropertyKey::String(s) => s.clone(),
    };
    ...
    let Some(export_name) = exports.get(&key).cloned() else {
        return Ok(ExecutionOutcome::Complete(None));
    };
    ...
    if let BindingName::Name(name) = binding.binding_name() {
        ...
        Ok(ExecutionOutcome::Complete(Some(target_module.namespace(context)?.into())))
    }
}
...
fn module_namespace_exotic_get(
    ...
    match module_namespace_exotic_get_interruptible(obj, key, receiver, context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
        ExecutionOutcome::Suspend(_) => Err(JsObject::suspended_internal_method_error(
            "property access",
        )),
...
fn module_namespace_exotic_get_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    receiver: JsValue,
    context: &mut InternalMethodPropertyContext<'_>,
    ...
    //     a. Return ! OrdinaryGet(O, P, Receiver).
    let key = match key {
        PropertyKey::Symbol(_) => return ordinary_get_interruptible(obj, key, receiver, context),
        PropertyKey::Index(idx) => js_string!(format!("{}", idx.get())),
        PropertyKey::String(s) => s.clone(),
    };
    ...
    if let BindingName::Name(name) = binding.binding_name() {
        ...
        // 9. If binding.[[BindingName]] is namespace, then
        //     a. Return GetModuleNamespace(targetModule).
        Ok(ExecutionOutcome::Complete(target_module.namespace(context)?.into()))
    }
}
...
fn module_namespace_exotic_set(
    ...
    match module_namespace_exotic_set_interruptible(_obj, _key, _value, _receiver, _context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
        ExecutionOutcome::Suspend(_) => Err(JsObject::suspended_internal_method_error(
            "property assignment",
        )),
...
#[allow(clippy::needless_pass_by_value, clippy::unnecessary_wraps)]
fn module_namespace_exotic_set_interruptible(
    _obj: &JsObject,
    _key: PropertyKey,
    _value: JsValue,
    _receiver: JsValue,
    ...
    // 1. Return false.
    Ok(ExecutionOutcome::Complete(false))
}

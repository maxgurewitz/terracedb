...
use boa_gc::{Finalize, Gc, GcAllocationBudgetExceeded, GcRefCell, Trace};
...
impl SyntheticModuleInitializer {
        ...
        F: Fn(&SyntheticModule, &mut Context) -> JsResult<()> + Copy + 'static,
    {
        ...
        unsafe { Self::try_from_closure(closure) }.expect(
            "synthetic module initializer allocation should not fail without an active allocation budget",
        )
    }
        ...
        T: Trace + 'static,
    {
        ...
        let ptr = Gc::into_raw(Gc::try_new(Callback {
            f: closure,
            captures,
        })?);
...
impl SyntheticModule {
    ...
    pub(super) fn link(&self, module_self: &Module, context: &mut Context) -> JsResult<()> {
        ...
        let cb = Gc::try_new(compiler.finish())?;

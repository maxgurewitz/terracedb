...
use boa_gc::GcAllocationBudgetExceeded;
...
/// Error thrown when the embedding runtime denies additional tracked memory.
#[derive(Debug, Clone, Eq, PartialEq, Error, Finalize)]
#[error("{message}")]
pub struct AllocationBudgetError {
    /// Number of bytes the runtime attempted to reserve.
    requested_bytes: usize,
    /// Human-readable failure message from the embedding runtime.
    message: String,
}

// SAFETY: this type only stores Rust-owned scalars and strings.
unsafe impl Trace for AllocationBudgetError {
    custom_trace!(_this, _mark, {});
}

impl AllocationBudgetError {
    /// Gets the number of bytes the runtime attempted to reserve.
    #[must_use]
    pub const fn requested_bytes(&self) -> usize {
        self.requested_bytes
    }

    /// Gets the human-readable failure message from the embedding runtime.
    #[must_use]
...
impl From<GcAllocationBudgetExceeded> for AllocationBudgetError {
    fn from(value: GcAllocationBudgetExceeded) -> Self {
        Self {
...
impl EngineError {
    ...
    fn into_erased(self, context: &mut Context) -> ErasedEngineError {
        match self {
            ...
            EngineError::AllocationBudget(err) => ErasedEngineError::AllocationBudget(err),
...
impl From<GcAllocationBudgetExceeded> for JsError {
    #[cfg_attr(feature = "native-backtrace", track_caller)]
    fn from(err: GcAllocationBudgetExceeded) -> Self {
        EngineError::from(AllocationBudgetError::from(err)).into()
    }
}
...
impl JsNativeError {
    ...
    /// ```
    #[inline]
    pub fn into_opaque(self, context: &mut Context) -> JsResult<JsObject> {
        let Self {
            kind,
            message,
            cause,
        ...
        let o = JsObject::try_from_proto_and_data_with_shared_shape(
            context.root_shape(),
            prototype,

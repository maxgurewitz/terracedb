...
impl JsFunction {
    ...
    pub(crate) fn empty_intrinsic_function(constructor: bool) -> Self {
        Self::try_empty_intrinsic_function(constructor).expect(
            "intrinsic function allocation should not fail without an active allocation budget",
        )
    }

    /// Creates a new, empty intrinsic function object with only its function internal methods set.
    pub(crate) fn try_empty_intrinsic_function(constructor: bool) -> JsResult<Self> {
        Ok(Self {
            inner: JsObject::try_from_proto_and_data(
                None,

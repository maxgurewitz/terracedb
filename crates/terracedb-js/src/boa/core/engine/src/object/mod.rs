...
pub use operations::{IntegrityLevel, InterruptibleCallOutcome};
...
impl<'realm> FunctionObjectBuilder<'realm> {
    ...
    pub fn try_build(self) -> JsResult<JsFunction> {
        let object = self.realm.intrinsics().templates().function().try_create(
        ...
        Ok(JsFunction::from_object_unchecked(object))
    }
}
...
impl<'ctx> ObjectInitializer<'ctx> {
    ...
    pub fn new(context: &'ctx mut Context) -> Self {
        Self::try_new(context)
            .expect("object allocation should not fail without an active allocation budget")
    }

    pub fn try_new(context: &'ctx mut Context) -> JsResult<Self> {
        let object = JsObject::try_with_object_proto(context.intrinsics())?;
    ...
    pub fn with_native_data<T: NativeObject>(data: T, context: &'ctx mut Context) -> Self {
        Self::try_with_native_data(data, context)
            .expect("object allocation should not fail without an active allocation budget")
    }

    pub fn try_with_native_data<T: NativeObject>(
        data: T,
        context: &'ctx mut Context,
    ) -> JsResult<Self> {
        let object = JsObject::try_from_proto_and_data_with_shared_shape(
            context.root_shape(),
    ...
    pub fn with_native_data_and_proto<T: NativeObject>(
        ...
        Self::try_with_native_data_and_proto(data, proto, context)
            .expect("object allocation should not fail without an active allocation budget")
    }

    pub fn try_with_native_data_and_proto<T: NativeObject>(
        data: T,
        proto: JsObject,
        context: &'ctx mut Context,
    ) -> JsResult<Self> {
        let object =
            JsObject::try_from_proto_and_data_with_shared_shape(context.root_shape(), proto, data)?
                .upcast();
        Ok(Self { context, object })
    }
    ...
    {
        self.try_function(function, binding, length)
            .expect("function allocation should not fail without an active allocation budget")
    }

    pub fn try_function<B>(
        &mut self,
        function: NativeFunction,
        binding: B,
        length: usize,
...
impl<'ctx> ConstructorBuilder<'ctx> {
    ...
    where
        B: Into<FunctionBinding>,
    {
        self.try_method(function, binding, length)
            .expect("function allocation should not fail without an active allocation budget")
    }
    ...
    where
        B: Into<FunctionBinding>,
    {
        self.try_static_method(function, binding, length)
            .expect("function allocation should not fail without an active allocation budget")
    }

    pub fn try_static_method<B>(
        &mut self,
        function: NativeFunction,
        binding: B,
        length: usize,
    ...
    pub fn build(self) -> StandardConstructor {
        self.try_build()
            .expect("constructor allocation should not fail without an active allocation budget")
    }

    pub fn try_build(mut self) -> JsResult<StandardConstructor> {
        ...
        let prototype = {
            ...
            JsObject::try_from_object_and_vtable(self.prototype, &ORDINARY_INTERNAL_METHODS)?
        };
        ...
        let constructor = {
            ...
            JsObject::try_from_object_and_vtable(constructor, internal_methods)?
        };

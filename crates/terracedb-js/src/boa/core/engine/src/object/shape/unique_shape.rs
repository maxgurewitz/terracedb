...
use boa_gc::{Finalize, Gc, GcAllocationBudgetExceeded, GcRefCell, Trace, WeakGc};
...
impl UniqueShape {
    ...
    pub(crate) fn new(prototype: JsPrototype, property_table: PropertyTableInner) -> Self {
        Self::try_new(prototype, property_table)
            .expect("shape allocation should not fail without an active allocation budget")
    }

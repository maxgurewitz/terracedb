...
use boa_gc::{Finalize, GcAllocationBudgetExceeded, Trace};
...
impl PropertyMap {
    ...
    pub fn from_prototype_unique_shape(prototype: JsPrototype) -> Self {
        Self::try_from_prototype_unique_shape(prototype)
            .expect("property-map allocation should not fail without an active allocation budget")
    }

    #[inline]
    pub fn try_from_prototype_unique_shape(
        prototype: JsPrototype,
    ) -> Result<Self, GcAllocationBudgetExceeded> {
        Ok(Self {
            indexed_properties: IndexedProperties::default(),
            shape: UniqueShape::try_new(prototype, PropertyTableInner::default())?.into(),
            storage: Vec::default(),
        })
    }

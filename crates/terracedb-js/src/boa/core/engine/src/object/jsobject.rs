...
use boa_gc::{self, Finalize, Gc, GcAllocationBudgetExceeded, GcRef, GcRefCell, GcRefMut, Trace};
...
impl JsObject {
    fn allocation_budget_error(error: GcAllocationBudgetExceeded) -> crate::JsError {
        error.into()
    }
    ...
    /// Creates a new ordinary object with its prototype set to the `Object` prototype.
    #[inline]
    pub fn try_default(intrinsics: &Intrinsics) -> JsResult<Self> {
        Self::try_with_object_proto(intrinsics)
    }

    /// Creates a new `JsObject` from its inner object and its vtable.
    pub(crate) fn try_from_object_and_vtable<T: NativeObject>(
        object: Object<T>,
        vtable: &'static InternalObjectMethods,
    ) -> JsResult<Self> {
        let inner = Gc::try_new(VTableObject {
            object: GcRefCell::new(object),
            vtable,
        })
        .map_err(Self::allocation_budget_error)?;

        Ok(JsObject { inner }.upcast())
    }
    ...
    pub(crate) fn from_object_and_vtable<T: NativeObject>(
        ...
        Self::try_from_object_and_vtable(object, vtable)
            .expect("JsObject allocation should not fail without an active allocation budget")
    }
    ...
    /// Creates a new ordinary object with its prototype set to the `Object` prototype.
    #[inline]
    pub fn try_with_object_proto(intrinsics: &Intrinsics) -> JsResult<Self> {
        Self::try_from_proto_and_data(
            intrinsics.constructors().object().prototype(),
            OrdinaryObject,
        )
    }
    ...
    /// Creates a new ordinary object, with its prototype set to null.
    #[inline]
    pub fn try_with_null_proto() -> JsResult<Self> {
        Self::try_from_proto_and_data(None, OrdinaryObject)
    }
    ...
    pub fn from_proto_and_data<O: Into<Option<Self>>, T: NativeObject>(
        ...
        Self::try_from_proto_and_data(prototype, data)
            .expect("JsObject allocation should not fail without an active allocation budget")
    }

    /// Creates a new object with the provided prototype and object data.
    pub fn try_from_proto_and_data<O: Into<Option<Self>>, T: NativeObject>(
        prototype: O,
        data: T,
    ) -> JsResult<Self> {
        ...
        let inner = Gc::try_new(VTableObject {
            object: GcRefCell::new(Object {
                ...
                properties: PropertyMap::try_from_prototype_unique_shape(prototype.into())
                    .map_err(Self::allocation_budget_error)?,
                extensible: true,
    ...
    pub(crate) fn from_proto_and_data_with_shared_shape<O: Into<Option<Self>>, T: NativeObject>(
        ...
        Self::try_from_proto_and_data_with_shared_shape(root_shape, prototype, data)
            .expect("JsObject allocation should not fail without an active allocation budget")
    }

    /// Creates a new object with the provided prototype and object data using a shared shape.
    pub(crate) fn try_from_proto_and_data_with_shared_shape<
        O: Into<Option<Self>>,
        T: NativeObject,
    >(
        root_shape: &RootShape,
...
impl<T: NativeObject> JsObject<T> {
    ...
    /// assert!(obj.prototype().is_none());
    /// ```
    pub fn try_new_unique<O: Into<Option<JsObject>>>(prototype: O, data: T) -> JsResult<Self> {
        let internal_methods = data.internal_methods();
        let inner = Gc::try_new(VTableObject {
            object: GcRefCell::new(Object {
                data: ObjectData::new(data),
                properties: PropertyMap::try_from_prototype_unique_shape(prototype.into())
                    .map_err(JsObject::allocation_budget_error)?,
                extensible: true,
                private_elements: ThinVec::new(),
            }),
            vtable: internal_methods,
    ...
    pub fn new_unique<O: Into<Option<JsObject>>>(prototype: O, data: T) -> Self {
        Self::try_new_unique(prototype, data)
            .expect("JsObject allocation should not fail without an active allocation budget")
    }

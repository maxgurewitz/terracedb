...
use crate::{
    ...
    value::JsValue,
    vm::{CompletionRecord, source_info::NativeSourceInfo},
};
...
impl JsObject {
    #[inline]
    pub(crate) fn suspended_internal_method_error(operation: &str) -> crate::JsError {
        JsNativeError::error()
            .with_message(format!("{operation} suspended during synchronous execution"))
            .into()
    }
    ...
    pub(crate) fn __has_property__(
        ...
        match self.__has_property_interruptible__(key, context)? {
    ...
    pub(crate) fn __has_property_interruptible__(
        &self,
        key: &PropertyKey,
        context: &mut InternalMethodPropertyContext<'_>,
    ) -> JsResult<ExecutionOutcome<bool>> {
        (self.vtable().__has_property_interruptible__)(self, key, context)
    }
    ...
    pub(crate) fn __try_get__(
        ...
        match self.__try_get_interruptible__(key, receiver, context)? {
            ExecutionOutcome::Complete(value) => Ok(value),
            ExecutionOutcome::Suspend(_) => Err(Self::suspended_internal_method_error(
                "property access",
            )),
    ...
    pub(crate) fn __try_get_interruptible__(
        &self,
        key: &PropertyKey,
        receiver: JsValue,
        context: &mut InternalMethodPropertyContext<'_>,
    ) -> JsResult<ExecutionOutcome<Option<JsValue>>> {
        (self.vtable().__try_get_interruptible__)(self, key, receiver, context)
    }
    ...
    pub(crate) fn __get_interruptible__(
        &self,
        key: &PropertyKey,
        receiver: JsValue,
        context: &mut InternalMethodPropertyContext<'_>,
    ...
    pub(crate) fn __set__(
        ...
        match self.__set_interruptible__(key, value, receiver, context)? {
            ExecutionOutcome::Complete(value) => Ok(value),
            ExecutionOutcome::Suspend(_) => Err(Self::suspended_internal_method_error(
                "property assignment",
            )),
    ...
    pub(crate) fn __set_interruptible__(
        &self,
        key: PropertyKey,
        value: JsValue,
        receiver: JsValue,
        ...
        (self.vtable().__set_interruptible__)(self, key, value, receiver, context)
    }
...
pub(crate) const ORDINARY_INTERNAL_METHODS: InternalObjectMethods = InternalObjectMethods {
    ...
    __has_property_interruptible__: ordinary_has_property_interruptible,
    __try_get__: ordinary_try_get,
    __try_get_interruptible__: ordinary_try_get_interruptible,
    __get__: ordinary_get,
    __get_interruptible__: ordinary_get_interruptible,
    __set__: ordinary_set,
    __set_interruptible__: ordinary_set_interruptible,
...
pub struct InternalObjectMethods {
    ...
    pub(crate) __has_property_interruptible__: fn(
        &JsObject,
        &PropertyKey,
        &mut InternalMethodPropertyContext<'_>,
    ) -> JsResult<ExecutionOutcome<bool>>,
    ...
    pub(crate) __get_interruptible__: fn(
        &JsObject,
        &PropertyKey,
        JsValue,
        &mut InternalMethodPropertyContext<'_>,
    ...
    pub(crate) __try_get_interruptible__: fn(
        &JsObject,
        &PropertyKey,
        JsValue,
        &mut InternalMethodPropertyContext<'_>,
    ...
    pub(crate) __set_interruptible__: fn(
        &JsObject,
        PropertyKey,
        JsValue,
        JsValue,
...
pub(crate) enum CallValue {
    ...
    /// Calling must be resumed later by the embedder runtime.
    Suspended(NativeResume),
}
...
impl CallValue {
    ...
    #[cfg_attr(feature = "native-backtrace", track_caller)]
    pub(crate) fn resolve(mut self, context: &mut Context) -> JsResult<ResolvedCallValue> {
        loop {
            match self {
                CallValue::Ready => return Ok(ResolvedCallValue::Ready),
                CallValue::Complete => return Ok(ResolvedCallValue::Complete),
                CallValue::Suspended(continuation) => {
                    return Ok(ResolvedCallValue::Suspended(continuation));
                }
                CallValue::Pending {
                    func,
                    object,
...
pub(crate) fn ordinary_has_property(
    ...
    match ordinary_has_property_interruptible(obj, key, context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
...
pub(crate) fn ordinary_has_property_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    context: &mut InternalMethodPropertyContext<'_>,
) -> JsResult<ExecutionOutcome<bool>> {
    ...
    if obj.__get_own_property__(key, context)?.is_some() {
        Ok(ExecutionOutcome::Complete(true))
    } else {
        ...
        parent
            ...
            // 6. Return false.
            .map_or(Ok(ExecutionOutcome::Complete(false)), |obj| {
                obj.__has_property_interruptible__(key, context)
            })
    }
}
...
pub(crate) fn ordinary_get(
    ...
    match ordinary_get_interruptible(obj, key, receiver, context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
        ExecutionOutcome::Suspend(_) => Err(JsObject::suspended_internal_method_error(
            "property access",
        )),
...
pub(crate) fn ordinary_get_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    receiver: JsValue,
    context: &mut InternalMethodPropertyContext<'_>,
    ...
    match obj.__get_own_property__(key, context)? {
        ...
        None => {
            ...
            if let Some(parent) = obj.__get_prototype_of__(context)? {
                ...
                parent.__get_interruptible__(key, receiver, context)
            }
            // b. If parent is null, return undefined.
            else {
                Ok(ExecutionOutcome::Complete(JsValue::undefined()))
            }
        }
        Some(ref desc) => {
            match desc.kind() {
                ...
                DescriptorKind::Data {
                    value: Some(value), ..
                } => Ok(ExecutionOutcome::Complete(value.clone())),
                ...
                DescriptorKind::Accessor { get: Some(get), .. } if !get.is_undefined() => {
                    ...
                    match get
                        .as_object()
                        .js_expect("getter should be an object")
                        ?
                ...
                _ => Ok(ExecutionOutcome::Complete(JsValue::undefined())),
            }
        }
    }
}
...
pub(crate) fn ordinary_try_get(
    ...
    match ordinary_try_get_interruptible(obj, key, receiver, context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
        ExecutionOutcome::Suspend(_) => Err(JsObject::suspended_internal_method_error(
            "property access",
        )),
...
pub(crate) fn ordinary_try_get_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    receiver: JsValue,
    context: &mut InternalMethodPropertyContext<'_>,
    ...
    match obj.__get_own_property__(key, context)? {
        ...
        None => {
            ...
            if let Some(parent) = obj.__get_prototype_of__(context)? {
                ...
                // c. Return ? parent.[[Get]](P, Receiver).
                parent.__try_get_interruptible__(key, receiver, context)
            }
            // b. If parent is null, return undefined.
            else {
                Ok(ExecutionOutcome::Complete(None))
            }
        }
        Some(ref desc) => {
            match desc.kind() {
                ...
                DescriptorKind::Data {
                    value: Some(value), ..
                } => Ok(ExecutionOutcome::Complete(Some(value.clone()))),
                // 5. Assert: IsAccessorDescriptor(desc) is true.
                // 6. Let getter be desc.[[Get]].
                DescriptorKind::Accessor { get: Some(get), .. } if !get.is_undefined() => {
                    // 8. Return ? Call(getter, Receiver).
                    ...
                    {
                        InterruptibleCallOutcome::Complete(value) => {
                            Ok(ExecutionOutcome::Complete(Some(value)))
                        }
                        InterruptibleCallOutcome::Suspend(continuation) => {
                            Ok(ExecutionOutcome::Suspend(continuation))
                        }
                ...
                // 7. If getter is undefined, return undefined.
                _ => Ok(ExecutionOutcome::Complete(Some(JsValue::undefined()))),
            }
        }
    }
}
...
pub(crate) fn ordinary_set(
    ...
    match ordinary_set_interruptible(obj, key, value, receiver, context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
        ExecutionOutcome::Suspend(_) => Err(JsObject::suspended_internal_method_error(
            "property assignment",
        )),
...
pub(crate) fn ordinary_set_interruptible(
    obj: &JsObject,
    key: PropertyKey,
    value: JsValue,
    receiver: JsValue,
    ...
    // b. If parent is not null, then
        ...
        return parent.__set_interruptible__(key, value, receiver, context);
    }
    // c. Else,
    else {
    ...
    if own_desc.is_data_descriptor() {
        ...
        if !own_desc.expect_writable() {
            return Ok(ExecutionOutcome::Complete(false));
        }
        ...
        return receiver
            .create_data_property_with_slot(key, value, context)
            .map(ExecutionOutcome::Complete);
    }
    ...
    match own_desc.set() {
        Some(set) if !set.is_undefined() => {
            ...
            match set
                .as_object()
                .js_expect("setter should be an object")
                ?
                .call_interruptible(&receiver, &[value], context)?
            {
                InterruptibleCallOutcome::Complete(_) => {}
                InterruptibleCallOutcome::Suspend(continuation) => {
                    return Ok(ExecutionOutcome::Suspend(wrap_ordinary_set_suspend(
                        continuation,
                    )));
                }
            }
        ...
        _ => Ok(ExecutionOutcome::Complete(false)),
    }
}

fn wrap_ordinary_set_suspend(continuation: NativeResume) -> NativeResume {
    NativeResume::from_copy_closure_with_captures(resume_ordinary_set_suspend, continuation)
}

fn resume_ordinary_set_suspend(

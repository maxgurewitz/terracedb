...
impl Default for UriFunctions {
    fn default() -> Self {
        Self::try_default()
            .expect("URI intrinsic allocation should not fail without an active allocation budget")
    }
}
...
impl UriFunctions {
    pub(crate) fn try_default() -> JsResult<Self> {
        Ok(Self {
            decode_uri: JsFunction::try_empty_intrinsic_function(false)?,
            decode_uri_component: JsFunction::try_empty_intrinsic_function(false)?,
            encode_uri: JsFunction::try_empty_intrinsic_function(false)?,
            encode_uri_component: JsFunction::try_empty_intrinsic_function(false)?,
        })
    }

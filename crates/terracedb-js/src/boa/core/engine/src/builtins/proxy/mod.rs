...
use crate::{
    ...
    object::{
        InterruptibleCallOutcome, JsData, JsFunction, JsObject, JsPrototype,
        internal_methods::{
...
fn decode_optional_callable(value: JsValue) -> JsResult<Option<JsObject>> {
    match value.variant() {
        JsVariant::Undefined | JsVariant::Null => Ok(None),
        JsVariant::Object(_) if value
            .as_object()
            .expect("object variant must contain object")
            .is_callable() =>
        {
            Ok(Some(
                value
...
fn finalize_proxy_has_result(
    target: &JsObject,
    key: &PropertyKey,
    boolean_trap_result: bool,
    ...
    Ok(boolean_trap_result.into())
}

fn wrap_proxy_has_get_method_resume(
    ...
    NativeResume::from_copy_closure_with_captures(
        resume_proxy_has_after_get_method,
        (continuation, target, handler, key),
    )
}

fn resume_proxy_has_after_get_method(
    completion: crate::vm::CompletionRecord,
    captures: &(NativeResume, JsObject, JsObject, JsValue),
    context: &mut Context,
    ...
    match captures.0.call(completion, context)? {
        NativeFunctionResult::Value(value) => {
            ...
            match trap.call_interruptible(
                &captures.2.clone().into(),
                &[captures.1.clone().into(), captures.3.clone()],
                ...
                InterruptibleCallOutcome::Suspend(next) => Ok(NativeFunctionResult::Suspend(
                    wrap_proxy_has_trap_resume(next, captures.1.clone(), captures.3.clone()),
                )),
            }
        }
        NativeFunctionResult::Suspend(next) => Ok(NativeFunctionResult::Suspend(
            wrap_proxy_has_get_method_resume(
                next,
                captures.1.clone(),
                captures.2.clone(),
                captures.3.clone(),
...
fn wrap_proxy_has_trap_resume(
    continuation: NativeResume,
    target: JsObject,
    key: JsValue,
) -> NativeResume {
    NativeResume::from_copy_closure_with_captures(
        resume_proxy_has_after_trap_call,
        (continuation, target, key),
    )
}

fn resume_proxy_has_after_trap_call(
    completion: crate::vm::CompletionRecord,
    captures: &(NativeResume, JsObject, JsValue),
    context: &mut Context,
) -> JsResult<NativeFunctionResult> {
    match captures.0.call(completion, context)? {
        NativeFunctionResult::Value(value) => {
        ...
        NativeFunctionResult::Suspend(next) => Ok(NativeFunctionResult::Suspend(
            wrap_proxy_has_trap_resume(next, captures.1.clone(), captures.2.clone()),
        )),
    }
}

fn finalize_proxy_get_result(
    target: &JsObject,
    key: &PropertyKey,
    trap_result: JsValue,
    context: &mut InternalMethodPropertyContext<'_>,
...
fn wrap_proxy_get_get_method_resume(
    ...
    NativeResume::from_copy_closure_with_captures(
        resume_proxy_get_after_get_method,
        (continuation, target, handler, key, receiver),
    )
}

fn resume_proxy_get_after_get_method(
    completion: crate::vm::CompletionRecord,
    captures: &(NativeResume, JsObject, JsObject, JsValue, JsValue),
    context: &mut Context,
) -> JsResult<NativeFunctionResult> {
    match captures.0.call(completion, context)? {
        NativeFunctionResult::Value(value) => {
            ...
            let key = captures.3.to_property_key(&mut property_context)?;
            let Some(trap) = trap else {
                return match captures.1.__get_interruptible__(
                    &key,
                    captures.4.clone(),
                    &mut property_context,
                ) {
                    Ok(ExecutionOutcome::Complete(value)) => Ok(NativeFunctionResult::complete(value)),
                    Ok(ExecutionOutcome::Suspend(next)) => Ok(NativeFunctionResult::Suspend(next)),
                    Err(error) => Err(error),
                };
            };
            ...
            match trap.call_interruptible(
                &captures.2.clone().into(),
                &[captures.1.clone().into(), captures.3.clone(), captures.4.clone()],
                &mut property_context,
            )? {
                InterruptibleCallOutcome::Complete(value) => {
                    let value = finalize_proxy_get_result(
                        &captures.1,
                        &key,
                        value,
                        &mut property_context,
                ...
                InterruptibleCallOutcome::Suspend(next) => Ok(NativeFunctionResult::Suspend(
                    wrap_proxy_get_trap_resume(next, captures.1.clone(), captures.3.clone()),
                )),
            }
        }
        NativeFunctionResult::Suspend(next) => Ok(NativeFunctionResult::Suspend(
            wrap_proxy_get_get_method_resume(
                next,
                captures.1.clone(),
                captures.2.clone(),
                captures.3.clone(),
...
fn wrap_proxy_get_trap_resume(
    continuation: NativeResume,
    target: JsObject,
    key: JsValue,
) -> NativeResume {
    NativeResume::from_copy_closure_with_captures(
        resume_proxy_get_after_trap_call,
        (continuation, target, key),
    )
}

fn resume_proxy_get_after_trap_call(
    completion: crate::vm::CompletionRecord,
    captures: &(NativeResume, JsObject, JsValue),
    context: &mut Context,
) -> JsResult<NativeFunctionResult> {
    match captures.0.call(completion, context)? {
        NativeFunctionResult::Value(value) => {
            ...
            let key = captures.2.to_property_key(&mut property_context)?;
            let value =
                finalize_proxy_get_result(&captures.1, &key, value, &mut property_context)?;
            Ok(NativeFunctionResult::complete(value))
        }
        NativeFunctionResult::Suspend(next) => Ok(NativeFunctionResult::Suspend(
            wrap_proxy_get_trap_resume(next, captures.1.clone(), captures.2.clone()),
        )),
    }
}

fn finalize_proxy_set_result(
    target: &JsObject,
    key: &PropertyKey,
    value: &JsValue,
    boolean_trap_result: bool,
...
fn wrap_proxy_set_get_method_resume(
    continuation: NativeResume,
    target: JsObject,
    handler: JsObject,
    key: JsValue,
    ...
    NativeResume::from_copy_closure_with_captures(
        resume_proxy_set_after_get_method,
        (continuation, target, handler, key, value, receiver),
    )
}

fn resume_proxy_set_after_get_method(
    completion: crate::vm::CompletionRecord,
    captures: &(NativeResume, JsObject, JsObject, JsValue, JsValue, JsValue),
    context: &mut Context,
) -> JsResult<NativeFunctionResult> {
    match captures.0.call(completion, context)? {
        NativeFunctionResult::Value(value) => {
            ...
            let key = captures.3.to_property_key(&mut property_context)?;
            let Some(trap) = trap else {
                return match captures.1.__set_interruptible__(
                    key.clone(),
                    captures.4.clone(),
                    captures.5.clone(),
                    &mut property_context,
        ...
        NativeFunctionResult::Suspend(next) => Ok(NativeFunctionResult::Suspend(
            wrap_proxy_set_get_method_resume(
                next,
                captures.1.clone(),
                captures.2.clone(),
                captures.3.clone(),
...
fn wrap_proxy_set_trap_resume(
    ...
    NativeResume::from_copy_closure_with_captures(
        resume_proxy_set_after_trap_call,
        (continuation, target, key, value),
    )
}

fn resume_proxy_set_after_trap_call(
    completion: crate::vm::CompletionRecord,
    captures: &(NativeResume, JsObject, JsValue, JsValue),
    context: &mut Context,
) -> JsResult<NativeFunctionResult> {
    match captures.0.call(completion, context)? {
        NativeFunctionResult::Value(value) => {
...
impl JsData for Proxy {
    fn internal_methods(&self) -> &'static InternalObjectMethods {
        static BASIC: InternalObjectMethods = InternalObjectMethods {
            ...
            __has_property_interruptible__: proxy_exotic_has_property_interruptible,
            __try_get__: proxy_exotic_try_get,
            __try_get_interruptible__: proxy_exotic_try_get_interruptible,
            __get__: proxy_exotic_get,
            __get_interruptible__: proxy_exotic_get_interruptible,
            __set__: proxy_exotic_set,
            __set_interruptible__: proxy_exotic_set_interruptible,
...
pub(crate) fn proxy_exotic_has_property(
    ...
    match proxy_exotic_has_property_interruptible(obj, key, context)? {
...
pub(crate) fn proxy_exotic_try_get(
    ...
    match proxy_exotic_try_get_interruptible(obj, key, receiver, context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
        ExecutionOutcome::Suspend(_) => Err(JsObject::suspended_internal_method_error(
            "property access",
        )),
...
pub(crate) fn proxy_exotic_has_property_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    context: &mut InternalMethodPropertyContext<'_>,
    ...
    let trap = match handler.get_method_interruptible(js_string!("has"), context)? {
        ExecutionOutcome::Complete(trap) => trap,
        ExecutionOutcome::Suspend(continuation) => {
            return Ok(ExecutionOutcome::Suspend(wrap_proxy_has_get_method_resume(
                continuation,
                target.clone(),
                handler.clone(),
                key.clone().into(),
    ...
    let Some(trap) = trap else {
        return target.has_property_interruptible(key.clone(), context);
    };
    ...
    let boolean_trap_result = match trap.call_interruptible(
        ...
        InterruptibleCallOutcome::Suspend(continuation) => {
            return Ok(ExecutionOutcome::Suspend(wrap_proxy_has_trap_resume(
                continuation,
                target.clone(),
                key.clone().into(),
            )));
    ...
    let value = finalize_proxy_has_result(&target, key, boolean_trap_result, context)?;
    Ok(ExecutionOutcome::Complete(value.to_boolean()))
}

pub(crate) fn proxy_exotic_try_get_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    receiver: JsValue,
    context: &mut InternalMethodPropertyContext<'_>,
) -> JsResult<ExecutionOutcome<Option<JsValue>>> {
    match proxy_exotic_has_property_interruptible(obj, key, context)? {
        ExecutionOutcome::Complete(true) => {
            match proxy_exotic_get_interruptible(obj, key, receiver, context)? {
                ExecutionOutcome::Complete(value) => Ok(ExecutionOutcome::Complete(Some(value))),
                ExecutionOutcome::Suspend(continuation) => Ok(ExecutionOutcome::Suspend(continuation)),
            }
        }
        ExecutionOutcome::Complete(false) => Ok(ExecutionOutcome::Complete(None)),
        ExecutionOutcome::Suspend(continuation) => Ok(ExecutionOutcome::Suspend(continuation)),
    }
}
...
pub(crate) fn proxy_exotic_get_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    receiver: JsValue,
    context: &mut InternalMethodPropertyContext<'_>,
    ...
    let trap = match handler.get_method_interruptible(js_string!("get"), context)? {
        ExecutionOutcome::Complete(trap) => trap,
        ExecutionOutcome::Suspend(continuation) => {
            return Ok(ExecutionOutcome::Suspend(wrap_proxy_get_get_method_resume(
                continuation,
                target.clone(),
                handler.clone(),
                key.clone().into(),
    ...
    let Some(trap) = trap else {
        ...
        return target.__get_interruptible__(key, receiver, context);
    };

    // 7. Let trapResult be ? Call(trap, handler, « target, P, Receiver »).
    let trap_result = match trap.call_interruptible(
        &handler.into(),
        ...
        InterruptibleCallOutcome::Suspend(continuation) => {
            return Ok(ExecutionOutcome::Suspend(wrap_proxy_get_trap_resume(
                continuation,
                target.clone(),
                key.clone().into(),
            )));
    ...
    let value = finalize_proxy_get_result(&target, key, trap_result, context)?;
    Ok(ExecutionOutcome::Complete(value))
}
...
pub(crate) fn proxy_exotic_set(
    ...
    match proxy_exotic_set_interruptible(obj, key, value, receiver, context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
        ExecutionOutcome::Suspend(_) => Err(JsObject::suspended_internal_method_error(
            "property assignment",
        )),
...
pub(crate) fn proxy_exotic_set_interruptible(
    obj: &JsObject,
    key: PropertyKey,
    value: JsValue,
    receiver: JsValue,
    ...
    let trap = match handler.get_method_interruptible(js_string!("set"), context)? {
        ExecutionOutcome::Complete(trap) => trap,
        ExecutionOutcome::Suspend(continuation) => {
            return Ok(ExecutionOutcome::Suspend(wrap_proxy_set_get_method_resume(
                continuation,
                target.clone(),
                handler.clone(),
                key.clone().into(),
    ...
    let Some(trap) = trap else {
        ...
        return target.__set_interruptible__(key, value, receiver, context);
    };
    ...
    let boolean_trap_result = match trap.call_interruptible(
        ...
        InterruptibleCallOutcome::Complete(value) => value.to_boolean(),
        InterruptibleCallOutcome::Suspend(continuation) => {
            return Ok(ExecutionOutcome::Suspend(wrap_proxy_set_trap_resume(
                continuation,
                target.clone(),
                key.clone().into(),
                value.clone(),
    ...
    let value = finalize_proxy_set_result(&target, &key, &value, boolean_trap_result, context)?;
    Ok(ExecutionOutcome::Complete(value.to_boolean()))
}

...
impl IntrinsicObject for BuiltInFunctionObject {
    fn init(realm: &Realm) -> crate::JsResult<()> {
        ...
        BuiltInBuilder::from_standard_constructor::<Self>(realm)
            .method_native(
                NativeFunction::from_suspend_fn_ptr(Self::apply),
                js_string!("apply"),
                2,
            )
            .method(Self::bind, js_string!("bind"), 1)
            .method_native(
                NativeFunction::from_suspend_fn_ptr(Self::call),
                js_string!("call"),
                1,
            )
...
impl BuiltInFunctionObject {
    ...
    fn finish_interruptible_invoke(
        outcome: InterruptibleCallOutcome<JsValue>,
        context: &mut Context,
    ) -> JsResult<NativeFunctionResult> {
        match outcome {
    ...
    fn finish_interruptible_construct(
        outcome: InterruptibleCallOutcome<JsObject>,
        context: &mut Context,
    ) -> JsResult<NativeFunctionResult> {
        match outcome {
            InterruptibleCallOutcome::Complete(value) => {
                Ok(NativeFunctionResult::complete(JsValue::from(value)))
            }
            InterruptibleCallOutcome::Suspend(continuation) => {
                Ok(NativeFunctionResult::Suspend(continuation))
            }
    ...
    fn apply(
        ...
        if arg_array.is_null_or_undefined() {
            ...
            return Self::finish_interruptible_invoke(
                func.call_interruptible(this_arg, &[], context)?,
                context,
            );
        }
        ...
        Self::finish_interruptible_invoke(
            func.call_interruptible(this_arg, &arg_list, context)?,
            context,
        )
    }
    ...
    fn call(
        ...
        Self::finish_interruptible_invoke(
            func.call_interruptible(this_arg, args.get(1..).unwrap_or(&[]), context)?,
            context,
        )
    }
...
fn function_construct(
    ...
    let this = if code.is_derived_constructor() {
        ...
        let this = JsObject::try_from_proto_and_data_with_shared_shape(
            context.root_shape(),
            prototype,
            OrdinaryObject,
        )?

...
use crate::{
    ...
    object::{
        ...
        internal_methods::{
            ...
            ordinary_get_interruptible, ordinary_has_property, ordinary_has_property_interruptible,
            ordinary_prevent_extensions, ordinary_set, ordinary_set_interruptible, ordinary_try_get,
...
impl JsData for TypedArray {
    fn internal_methods(&self) -> &'static InternalObjectMethods {
        static METHODS: InternalObjectMethods = InternalObjectMethods {
            ...
            __has_property_interruptible__: typed_array_exotic_has_property_interruptible,
            __define_own_property__: typed_array_exotic_define_own_property,
            __try_get__: typed_array_exotic_try_get,
            __try_get_interruptible__: typed_array_exotic_try_get_interruptible,
            __get__: typed_array_exotic_get,
            __get_interruptible__: typed_array_exotic_get_interruptible,
            __set__: typed_array_exotic_set,
            __set_interruptible__: typed_array_exotic_set_interruptible,
...
pub(crate) fn typed_array_exotic_has_property(
    ...
    match typed_array_exotic_has_property_interruptible(obj, key, context)? {
...
pub(crate) fn typed_array_exotic_has_property_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    context: &mut InternalMethodPropertyContext<'_>,
    ...
    if let Some(numeric_index) = p {
        return is_valid_integer_index(obj, numeric_index).map(ExecutionOutcome::Complete);
    }

    // 2. Return ? OrdinaryHasProperty(O, P).
    ordinary_has_property_interruptible(obj, key, context)
}
...
pub(crate) fn typed_array_exotic_try_get(
    ...
    match typed_array_exotic_try_get_interruptible(obj, key, receiver, context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
        ExecutionOutcome::Suspend(_) => Err(JsObject::suspended_internal_method_error(
            "property access",
        )),
...
pub(crate) fn typed_array_exotic_try_get_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    receiver: JsValue,
    context: &mut InternalMethodPropertyContext<'_>,
    ...
    if let Some(numeric_index) = p {
        ...
        return typed_array_get_element(obj, numeric_index).map(ExecutionOutcome::Complete);
    }

    // 2. Return ? OrdinaryGet(O, P, Receiver).
    ordinary_try_get_interruptible(obj, key, receiver, context)
}
...
pub(crate) fn typed_array_exotic_get(
    ...
    match typed_array_exotic_get_interruptible(obj, key, receiver, context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
        ExecutionOutcome::Suspend(_) => Err(JsObject::suspended_internal_method_error(
            "property access",
        )),
...
pub(crate) fn typed_array_exotic_get_interruptible(
    obj: &JsObject,
    key: &PropertyKey,
    receiver: JsValue,
    context: &mut InternalMethodPropertyContext<'_>,
    ...
    if let Some(numeric_index) = p {
        ...
        return Ok(ExecutionOutcome::Complete(
            typed_array_get_element(obj, numeric_index)?.unwrap_or_default(),
        ));
    }

    // 2. Return ? OrdinaryGet(O, P, Receiver).
    ordinary_get_interruptible(obj, key, receiver, context)
}
...
pub(crate) fn typed_array_exotic_set(
    ...
    match typed_array_exotic_set_interruptible(obj, key, value, receiver, context)? {
        ExecutionOutcome::Complete(value) => Ok(value),
        ExecutionOutcome::Suspend(_) => Err(JsObject::suspended_internal_method_error(
            "property assignment",
        )),
...
pub(crate) fn typed_array_exotic_set_interruptible(
    obj: &JsObject,
    key: PropertyKey,
    value: JsValue,
    receiver: JsValue,
    ...
    if let Some(numeric_index) = p {
        ...
        if JsValue::same_value(&obj.clone().into(), &receiver) {
            ...
            // 2. Return true.
            return Ok(ExecutionOutcome::Complete(true));
        }
    ...
    ordinary_set_interruptible(obj, key, value, receiver, context)
}

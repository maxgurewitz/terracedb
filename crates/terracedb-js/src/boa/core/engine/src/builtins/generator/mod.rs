...
impl GeneratorContext {
    ...
    pub(crate) fn continue_with_value(
        &mut self,
        value: JsValue,
        context: &mut Context,
    ) -> CompletionRecord {
        ...
        let Some(frame) = self.call_frame.take() else {
            context.vm.stack.push(value);
            let result = context.run();
            if !matches!(result, CompletionRecord::Suspend) {
                std::mem::swap(&mut context.vm.stack, &mut self.stack);
            }
            return result;
        };
    ...
    pub(crate) fn continue_with_error(
        &mut self,
        err: JsError,
        context: &mut Context,
    ) -> CompletionRecord {
        std::mem::swap(&mut context.vm.stack, &mut self.stack);
        let Some(frame) = self.call_frame.take() else {
            let result = match context.handle_error(err) {
                ControlFlow::Continue(()) => context.run(),
                ControlFlow::Break(record) => record,
            };
            if !matches!(result, CompletionRecord::Suspend) {
                std::mem::swap(&mut context.vm.stack, &mut self.stack);
            }
    ...
    fn with_current<T>(&mut self, context: &mut Context, f: impl FnOnce(&mut Context) -> T) -> T {
        std::mem::swap(&mut context.vm.stack, &mut self.stack);
        let Some(frame) = self.call_frame.take() else {
            return f(context);
        };
    ...
    pub(crate) fn generator_resume(
        ...
        match record {
            ...
            CompletionRecord::Suspend => {
                r#gen.state = GeneratorState::Completed;
                Err(PanicError::new(
                    "generator resumed with suspended completion unexpectedly",
                )
                .into())
            }
        }

...
impl ArrayIterator {
    ...
    pub(crate) fn create_array_iterator(
        ...
        let array_iterator = JsObject::try_from_proto_and_data_with_shared_shape(
            context.root_shape(),
            context.intrinsics().objects().iterator_prototypes().array(),
            Self::new(array, kind),
        )?;
        Ok(array_iterator.into())
    }
    ...
    pub(crate) fn next(this: &JsValue, _: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
        ...
        match array_iterator.kind {
            PropertyNameKind::Key => create_iter_result_object(index.into(), false, context),
            PropertyNameKind::Value => {
                let element_value = array_iterator.array.get(index, context)?;
                create_iter_result_object(element_value, false, context)
            }

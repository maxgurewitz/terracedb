...
pub struct Vm {
    ...
    pub(crate) suspended_resume: Option<NativeResume>,
...
impl Vm {
    ...
    pub(crate) fn new(realm: Realm) -> Self {
        Self::try_new(realm)
            .expect("VM allocation should not fail without an active allocation budget")
    }

    /// Creates a new virtual machine.
    pub(crate) fn try_new(realm: Realm) -> JsResult<Self> {
        let mut frames = Vec::with_capacity(16);
        frames.push(CallFrame::new(
            Gc::try_new(CodeBlock::new(JsString::default(), 0, true))?,
            None,
...
impl Context {
    ...
    pub(crate) fn handle_error(&mut self, mut err: JsError) -> ControlFlow<CompletionRecord> {

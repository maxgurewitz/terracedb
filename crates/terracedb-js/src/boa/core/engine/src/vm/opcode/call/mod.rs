...
fn wrap_call_resume(continuation: NativeResume) -> NativeResume {
    NativeResume::from_copy_closure_with_captures(resume_call_operation, continuation)
}

fn resume_call_operation(
    ...
    pub(super) fn operation(
        ...
        if JsObject::equals(&object, &eval) {
            ...
            if let Some(x) = arguments.first() {
                ...
                let result = match crate::builtins::eval::Eval::perform_eval(
                    x,
                    true,
                    Some(scope),
                    strict,
        ...
        match object.__call__(argument_count.into()).resolve(context) {
            Ok(ResolvedCallValue::Ready | ResolvedCallValue::Complete) => ControlFlow::Continue(()),
            Ok(ResolvedCallValue::Suspended(continuation)) => {
                context.install_interruptible_resume(wrap_call_resume(continuation));
                ControlFlow::Break(crate::vm::CompletionRecord::Suspend)
            }
...
impl CallEvalSpread {
    ...
    pub(super) fn operation(
        ...
        match object.__call__(argument_count).resolve(context) {
            Ok(ResolvedCallValue::Ready | ResolvedCallValue::Complete) => ControlFlow::Continue(()),
            Ok(ResolvedCallValue::Suspended(continuation)) => {
                context.install_interruptible_resume(wrap_call_resume(continuation));
                ControlFlow::Break(crate::vm::CompletionRecord::Suspend)
...
impl Call {
    ...
    pub(super) fn operation(
        ...
        let Some(object) = func.as_object() else {
            return context.handle_error(Self::handle_not_callable());
        };
...
impl CallSpread {
    #[inline(always)]
    pub(super) fn operation((): (), context: &mut Context) -> ControlFlow<crate::vm::CompletionRecord> {
        // Get the arguments that are stored as an array object on the stack.
        let arguments_array = context.vm.stack.pop();
        let arguments_array_object = arguments_array
            .as_object()

use crate::{
    Allocator, Ephemeron, GcAllocationBudgetExceeded, GcErasedPointer, Tracer, WeakGc,
...
impl<T: Trace + ?Sized> Gc<T> {
    ...
    pub fn try_new(value: T) -> Result<Self, GcAllocationBudgetExceeded>
    where
        T: Sized,
    {
        let inner_ptr = Allocator::try_alloc_gc(GcBox::new(value))?;
    ...
    {
        Self::try_new(value)
            .expect("GC allocation should not fail without an active allocation budget")
    }

    pub fn try_new_cyclic<F>(data_fn: F) -> Result<Self, GcAllocationBudgetExceeded>
    where
        F: FnOnce(&WeakGc<T>) -> T,
        T: Sized,
    {
        let weak = unsafe {
            Ephemeron::from_raw(Allocator::try_alloc_ephemeron(EphemeronBox::new_empty())?).into()
        };

        let gc = Self::try_new(data_fn(&weak))?;

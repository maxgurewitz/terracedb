...
use crate::{
    Allocator, Ephemeron, Finalize, Gc, GcAllocationBudgetExceeded, GcRef, GcRefCell, Trace,
    custom_trace,
};
...
impl<K: Trace + ?Sized, V: Trace> WeakMap<K, V> {
    ...
    #[inline]
    pub fn try_new() -> Result<Self, GcAllocationBudgetExceeded> {
        Allocator::register_weak_map(Allocator::try_alloc_weak_map()?)
    }
    ...
    #[inline]
    pub fn new() -> Self {
        Self::try_new().expect("GC allocation should not fail without an active allocation budget")
    }

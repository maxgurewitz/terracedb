use crate::{Ephemeron, Finalize, Gc, GcAllocationBudgetExceeded, Trace};
...
impl<T: Trace + ?Sized> WeakGc<T> {
    ...
    #[inline]
    pub fn try_new(value: &Gc<T>) -> Result<Self, GcAllocationBudgetExceeded> {
        Ok(Self {
            inner: Ephemeron::try_new(value, ())?,
        })
    }

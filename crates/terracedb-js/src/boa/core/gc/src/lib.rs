...
thread_local!(static GC_ALLOCATION_BUDGET: RefCell<Option<Arc<dyn GcAllocationBudget>>> = RefCell::new(None));
...
/// Error returned when an allocation budget refuses to reserve more memory.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GcAllocationBudgetExceeded {
    /// Number of additional bytes the allocator tried to reserve.
    pub requested_bytes: usize,
    /// Human-readable error message from the embedding runtime.
    pub message: String,
}

impl std::fmt::Display for GcAllocationBudgetExceeded {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(&self.message)
    }
}

impl std::error::Error for GcAllocationBudgetExceeded {}

/// Hook for embedding runtimes to charge and release GC-managed bytes.
pub trait GcAllocationBudget: Send + Sync {
    /// Reserve `bytes` more bytes before a GC allocation is committed.
    fn reserve_bytes(&self, bytes: usize) -> Result<(), GcAllocationBudgetExceeded>;

    /// Release `bytes` when GC-managed allocations are freed.
    fn release_bytes(&self, bytes: usize);
}

/// Guard that restores the previously installed allocation budget on drop.
pub struct GcAllocationBudgetGuard(Option<Arc<dyn GcAllocationBudget>>);

impl Drop for GcAllocationBudgetGuard {
    fn drop(&mut self) {
        GC_ALLOCATION_BUDGET.with(|slot| {
            *slot.borrow_mut() = self.0.take();
        });
    }
}

/// Installs an allocation budget for the current thread and returns a guard that restores the
/// previous budget on drop.
#[must_use]
pub fn set_allocation_budget(
    budget: Option<Arc<dyn GcAllocationBudget>>,
) -> GcAllocationBudgetGuard {
    let previous = GC_ALLOCATION_BUDGET.with(|slot| slot.replace(budget));
    GcAllocationBudgetGuard(previous)
}

/// Runs `f` with the provided allocation budget installed for the current thread.
pub fn with_allocation_budget<R>(
    budget: Option<Arc<dyn GcAllocationBudget>>,
    f: impl FnOnce() -> R,
) -> R {
    let _guard = set_allocation_budget(budget);
    f()
}

fn try_reserve_budget_bytes(bytes: usize) -> Result<(), GcAllocationBudgetExceeded> {
    if bytes == 0 {
        return Ok(());
    }
    GC_ALLOCATION_BUDGET.with(|slot| {
...
fn release_budget_bytes(bytes: usize) {
    if bytes == 0 {
        return;
    }
    GC_ALLOCATION_BUDGET.with(|slot| {
...
impl Allocator {
    fn try_alloc_gc<T: Trace>(
        value: GcBox<T>,
    ) -> Result<NonNull<GcBox<T>>, GcAllocationBudgetExceeded> {
        let element_size = size_of_val::<GcBox<T>>(&value);
        BOA_GC.with(|st| {
            let mut gc = st.borrow_mut();

            Self::manage_state(&mut gc);
            try_reserve_budget_bytes(element_size)?;
    ...
    fn try_alloc_ephemeron<K: Trace + ?Sized, V: Trace>(
        value: EphemeronBox<K, V>,
    ) -> Result<NonNull<EphemeronBox<K, V>>, GcAllocationBudgetExceeded> {
    ...
    fn try_alloc_weak_map<K: Trace + ?Sized, V: Trace>()
    -> Result<WeakMap<K, V>, GcAllocationBudgetExceeded> {
        Ok(WeakMap {
            inner: Gc::try_new(GcRefCell::new(RawWeakMap::new()))?,
        })
    }

    fn register_weak_map<K: Trace + ?Sized, V: Trace>(
        weak_map: WeakMap<K, V>,
    ) -> Result<WeakMap<K, V>, GcAllocationBudgetExceeded> {
        let weak = WeakGc::try_new(&weak_map.inner)?;
        ...
        weak.retain(|eph| {
            ...
            if header.is_marked() {
                ...
                *total_allocated -= unallocated_bytes;
                release_budget_bytes(unallocated_bytes);
...
/// Returns a snapshot of the current thread-local Boa GC runtime state.
#[must_use]
pub fn runtime_stats() -> GcRuntimeStats {
    BOA_GC.with(|current| {
        ...
        GcRuntimeStats {
            ...
            strong_count: gc.strongs.len(),
            weak_count: gc.weaks.len(),
            weak_map_count: gc.weak_maps.len(),
        }
    })
}

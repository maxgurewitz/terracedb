...
const bytes = (value) => Array.from(String(value)).map((char) => char.charCodeAt(0));
const text = (value) => String.fromCharCode(...value);

const schemaType = (name, parse) => ({ name, parse });

const parseSchema = (schema, value, path = "value") => {
  ...
  object(shape) {
    return schemaType("object", (value, path) => {
      ...
      for (const [key, inner] of Object.entries(shape)) {
        result[key] = parseSchema(inner, value[key], `${path}.${key}`);
      }
...
const normalizeVisibility = (value) => {
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw invalid("workflow visibility must be an object");
  }
  const summary = value.summary ?? {};
  if (!summary || typeof summary !== "object" || Array.isArray(summary)) {
    throw invalid("workflow visibility summary must be an object");
  }
...
const wf = {
  ...
  spawn() {
    unsupported("wf.spawn(...)");
  },
};

export { bytes, schema, text, wf };

...
use crate::{
    ...
    OpenError, PendingWork, PendingWorkBudget, PendingWorkType, ReadError, Rng, S3Location,
...
async fn forced_flush_guardrails_still_honor_domain_rate_limit_delay() {
    ...
    assert!(
        throttled_snapshot.throttled_writes_by_domain[&profile.foreground.domain] > 0,
        "rate-limited write should publish throttling before the delay elapses"
    );

    clock.advance(Duration::from_millis(350));
    let forced_snapshot = updates
        .wait_for(|snapshot| snapshot.forced_flushes >= 1)
        .await
        .expect("forced flush update");
    assert!(
        !write.is_finished(),
    ...
    let final_snapshot = updates.current();
    assert!(final_snapshot.forced_flushes >= 1);
    assert!(
        final_snapshot
            .throttled_writes_by_domain
            .get(&profile.foreground.domain)
        ...
        .wait_for(|snapshot| {
            snapshot
                ...
                .is_some_and(|recorded| {
                    recorded.diagnostics.level == crate::AdmissionPressureLevel::RateLimit
                        && recorded
                            .diagnostics
                            .triggered_by
                            .contains(&crate::AdmissionPressureSignal::MutableBudget)
...
async fn multi_table_commit_waits_for_the_slowest_rate_limited_table() {
    ...
    let fast_only_snapshot = updates
        .wait_for(|snapshot| {
            snapshot
                .current_admission_diagnostics_by_domain
                .get(&domain)
        ...
        .await
        .expect("rate-limited fast-only commit");
    assert!(
    ...
    let mixed_snapshot = updates
        ...
        .await
        .expect("rate-limited mixed commit");
    assert!(
        !mixed.is_finished(),
...
async fn columnar_sstables_round_trip_all_supported_field_types_and_tombstones() {
    ...
    assert_eq!(layout.decode_metadata.schema_version, schema.version);
    assert_eq!(
        layout
            .decode_metadata
            .fields
    ...
    assert!(
        layout
            .substreams
            .iter()
            .any(|substream| substream.compression == crate::ColumnarCompression::None)
    );
    assert!(
        layout
            .substreams
            .iter()
            .any(|substream| substream.compression == crate::ColumnarCompression::Lz4)
    );
    assert!(
        layout
            .substreams
            .iter()
            .any(|substream| substream.compression == crate::ColumnarCompression::Zstd)
    );
...
#[tokio::test]
async fn columnar_decode_metadata_mismatch_fails_closed_on_reopen() {
    let file_system = Arc::new(crate::StubFileSystem::default());
    let object_store = Arc::new(StubObjectStore::default());
    let dependencies = dependencies(file_system.clone(), object_store);
    ...
    let schema = config.schema.clone().expect("columnar schema");
    let db = Db::open(
        tiered_config("/columnar-decode-metadata-mismatch"),
        dependencies.clone(),
    )
    .await
    .expect("open db");
    ...
    table
        ...
        .await
        .expect("write row");
    db.flush().await.expect("flush columnar sstable");
    ...
    let (mut footer, footer_start) =
        Db::columnar_footer_from_bytes(&live.meta.file_path, &file_bytes).expect("decode footer");
    let layout = &mut footer.layout;
    let decode_field = layout
        .decode_metadata
        .fields
        .iter_mut()
...
async fn columnar_cache_usage_subscription_matches_snapshot_after_cache_population() {
    ...
    let published = updates
        .wait_for(|snapshot| snapshot != &initial)
        .await
...
async fn group_commit_batches_waiters_under_one_sync() {
    ...
    let reserved = progress
        .wait_for(|snapshot| snapshot.reserved_sequence >= SequenceNumber::new(2))
        .await
        .expect("db progress update");
    assert_eq!(reserved.reserved_sequence, SequenceNumber::new(2));
    ...
    assert_eq!(
        db.progress_snapshot().reserved_sequence,
        SequenceNumber::new(2)
    );
}
...
async fn group_commit_sync_failure_cascades_across_multiple_sealed_batches() {
    ...
    let reserved = progress
        .wait_for(|snapshot| snapshot.reserved_sequence >= SequenceNumber::new(3))
        .await
        .expect("db progress update");
    assert_eq!(reserved.reserved_sequence, SequenceNumber::new(3));
